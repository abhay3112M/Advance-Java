...
public class BankSpecs {
	...
	final int accountType = 1; //1. Saving Account  2. Current Account   3. OverDraftAccount
	...
	public void creditInterstShouldCreditInterstToAllAccounts() {
		bank.creditInterest();
		double rate1 = bank.getInterest(existingAccount1,correctPassword);
	...
	@Test
	public void withdrwShouldFailInSavingAccountMinimum500Req() {
		Response response = bank.withdraw(existingAccount1,(int)initialBalance,correctPassword);
		assertTrue(response.getCode()==ResponseStatus.INSUFFICIENT_FUNDS);
